...
float vDot2(Vector3 a, Vector3 b) { return a.x * b.x + a.y * b.y + a.z * b.z; }

float vLength2(Vector3 v) { return sqrtf(v.x * v.x + v.y * v.y + v.z * v.z); }
...
Vector3 vMulByScalar2(Vector3 a, float scalar) {
...
Vector3 vSubVectors2(Vector3 a, Vector3 b) {
  return (Vector3){a.x - b.x, a.y - b.y, a.z - b.z};
}

Vector3 vSqrt(Vector3 a) {
...
Vector3 vRefract(Vector3 i, Vector3 n, float eta) {
  float cosi = -vDot2(n, i);
  float k = 1.0f - eta * eta * (1.0f - cosi * cosi);
  if (k < 0.0f) {
    return (Vector3){0, 0, 0};
  }
  return vAddVectors3(vMulByScalar2(i, eta),
                      vMulByScalar2(n, eta * cosi - sqrtf(k)));
}
